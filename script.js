...
let resultsChart = new Chart(ctx, {
  ...
  data: {
      ...
      datasets: [{
          label: 'Votes',
          data: [votes.A, votes.B, votes.C, votes.D],
          backgroundColor: [
              '#3498db',
              '#ff00c8',
              '#2ecc71',
...
function updateResults() {
  resultsChart.data.datasets[0].data = [votes.A, votes.B, votes.C, votes.D];
  resultsChart.update();
...
function showDetailedResults() {
  ...
  document.getElementById('votesD').textContent = votes.D;
  ...
  document.getElementById('percentD').textContent = totalVotes > 0 ? ((votes.D / totalVotes) * 100).toFixed(2) + '%' : '0%';
